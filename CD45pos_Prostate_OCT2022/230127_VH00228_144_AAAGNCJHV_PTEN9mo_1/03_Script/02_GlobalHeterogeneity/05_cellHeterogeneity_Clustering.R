...
print(
  DimPlot( merge_seurat_object, 
           reduction = ifelse(exists("useReduction"), useReduction, "umap"), 
           group.by = "seurat_clusters", split.by = "sample.id", ncol = 2,
           label = TRUE) + 
    ggtitle( "Map of cells with clusters")
)

origin_vs_cluster_table = table( merge_seurat_object$sample.id, merge_seurat_object$seurat_clusters)
...
print(
  DimPlot( merge_seurat_object, 
           reduction = ifelse(exists("useReduction"), useReduction, "umap"), 
           group.by = "seurat_clusters", split.by = "sample.condition", ncol = 2,
           label = TRUE) + 
    ggtitle( "Map of cells with clusters")
)

condition_vs_cluster_table = table( merge_seurat_object$sample.condition, merge_seurat_object$seurat_clusters)
