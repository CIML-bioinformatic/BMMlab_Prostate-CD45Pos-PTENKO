...
umap_analysis <- scDEED( sc10x@assays$RNA@counts , num_pc = nbPC_dimreduc, use_method = "umap",visualization = TRUE)
...
cat("<BR>Best Neighbors number for UMAP =", best_UMAP_n)
cat("<BR>Best Min distance number for UMAP =", best_UMAP_mindist)
cat("<BR><BR>")

# Run the UMAP with the optimized parameters
sc10x = RunUMAP( sc10x, dims = 1:nbPC_dimreduc, n.neighbors = best_UMAP_n.neighbors, min.dist = best_UMAP_min.dist)
...
DimPlot( sc10x, reduction = useReduction, group.by = "scDblFinder") + 
