...
# This function uses the silhouette score to suggest reassignment of the cells
# If a cell has a negative silhouette score, its means the cells seems not assigned
# to the best cluster. The new cluster suggested is the nereast one in term of silhouette.
optimize_clustering <- function( sobject,
                                 cluster_col = "seurat_clusters",
                                 dims = c(1:10)) {
  
  silhouette_result = silhouette_seurat( sobject, cluster_col, dims)
  
  sobject = AddMetaData( sobject, metadata = silhouette_result[ , "neighbor"], col.name = "sil_neighbor")
  sobject = AddMetaData( sobject, metadata = silhouette_result[ , "sil_width"], col.name = "sil_width")
  
  suggested_cluster = vector()
  for( sil_index in 1:nrow( silhouette_result)){
    if( silhouette_result[ sil_index, "sil_width"] < 0){
      suggested_cluster = append( suggested_cluster, silhouette_result[ sil_index, "neighbor"])
    }else{
      suggested_cluster = append( suggested_cluster, silhouette_result[ sil_index, "cluster"])
    }
  }
  
  suggested_cluster = unname( suggested_cluster)
  
  sobject = AddMetaData( sobject, metadata = suggested_cluster, col.name = "sil_seurat_clusters")
...
# ########################################################
# Convert a list of Mouse genes to their homolog in Human
# ########################################################

convertMouseGeneList <- function(x){
  
  genesV2 = babelgene::orthologs( genes = x, human = FALSE, species = "mouse")
...
plotViolinJitter = function(plotData, xAxisFormula, yAxisFormula, colorFormula = NULL, pointSize = 3, pointsColorFormula = "#444444", pointsOpacity = 0.5, xAxisTitle = "", yAxisTitle = "", hoverText = NULL, traceName = "", xTicklabels = c(""), panelWidth = 300, panelHeight = 600, thresholdHigh = NULL, thresholdLow = NULL)
{
  ...
  cat( sum( !is.numeric( constantJitter)))
  ...
  plotData = cbind(plotData, jitterCoords = constantJitter);
  ...
  p = plot_ly( plotData,
    ...
    add_trace( type = "scatter",
               mode = "markers",
               x = update(xAxisFormula, as.formula( "~ . + jitterCoords")), # Update formula adding jitter values
