...
Idents( sc10x) = paste0( "RNA_snn_res.", FINDCLUSTERS_RESOLUTION)
sc10x = AddMetaData( sc10x, metadata = Idents( sc10x), col.name = "seurat_clusters")
...
Idents( sc10x) = "seurat_clusters"
clustersColor = scales::hue_pal()( base::nlevels( Idents( sc10x)))
...
geneExpByCluster = do.call( rbind, 
                            apply( as.matrix( LayerData( sc10x)), # expression values
                                   1,                                # by rows
