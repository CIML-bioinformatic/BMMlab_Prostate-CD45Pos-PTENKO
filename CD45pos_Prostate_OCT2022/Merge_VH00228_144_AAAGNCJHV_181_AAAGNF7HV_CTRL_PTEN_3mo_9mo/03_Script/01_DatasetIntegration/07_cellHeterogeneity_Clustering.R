...
# ..........................................................................................................
## @knitr heterogeneity_identifyClusters_makeClusters_chosenresolution
# ..........................................................................................................
...
cellsData = cbind( "Cell" = colnames( merge_seurat_object), # cell names from rownames conflict with search field in datatable
                   ...
                   "percent.mito" = if(length( mito.genes)) as.numeric(format(merge_seurat_object[["percent.mito", drop = TRUE]], digits = 5)) else NULL,
                   "percent.ribo" = if(length( ribo.genes)) as.numeric(format(merge_seurat_object[["percent.ribo", drop = TRUE]], digits = 5)) else NULL,
...
cat(" \n \n")
cat("### UMAP of clusters vs sample")
cat(" \n \n")
...
print(
  DimPlot( merge_seurat_object, 
           ...
           label = TRUE) + 
    ggtitle( "Map of cells with clusters vs origin")
)
...
cat(" \n \n")
cat("### UMAP of clusters vs condition")
cat(" \n \n")
...
print(
  DimPlot( merge_seurat_object, 
           ...
           label = TRUE) + 
    ggtitle( "Map of cells with clusters vs condition")
)
...
cat(" \n \n")
cat("### UMAP of clusters vs date")
cat(" \n \n")

# Plot the map with clusters with ggplot vs date of origin
print(
  DimPlot( merge_seurat_object, 
           reduction = ifelse(exists("useReduction"), useReduction, "umap"), 
           group.by = "seurat_clusters", split.by = "orig.date", ncol = 2,
           label = TRUE) + 
    ggtitle( "Map of cells with clusters vs date")
)

date_vs_cluster_table = table( merge_seurat_object$orig.date, merge_seurat_object$seurat_clusters)
date_vs_cluster_table %>% kable( caption = "Cell distribution of Date versus Clusters") %>% kable_styling()

origindate_vs_cluster_chisq_test = chisq.test( date_vs_cluster_table)
corrplot::corrplot( origindate_vs_cluster_chisq_test$residuals, is.cor = FALSE)

cat(" \n \n")
cat("### UMAP of clusters vs condition x date")
cat(" \n \n")
...
print(
  DimPlot( merge_seurat_object, 
           reduction = ifelse(exists("useReduction"), useReduction, "umap"), 
           group.by = "seurat_clusters", split.by = "orig.condition_date", ncol = 2,
           label = TRUE) + 
    ggtitle( "Map of cells with clusters vs date")
)

condition_date_vs_cluster_table = table( merge_seurat_object$orig.condition_date, merge_seurat_object$seurat_clusters)
condition_date_vs_cluster_table %>% kable( caption = "Cell distribution of Condition x Date versus Clusters") %>% kable_styling()

originconditiondate_vs_cluster_chisq_test = chisq.test( condition_date_vs_cluster_table)
corrplot::corrplot( originconditiondate_vs_cluster_chisq_test$residuals, is.cor = FALSE)

# Plot the map with clusters with ggplot vs outlier status
if( QC_EXPLORATION_MODE == TRUE){
  
  cat(" \n \n")
  cat("### UMAP of clusters vs outliers")
  cat(" \n \n")
  
  print(
    DimPlot( merge_seurat_object, 
             reduction = ifelse(exists("useReduction"), useReduction, "umap"), 
             group.by = "seurat_clusters", split.by = "outlier", ncol = 2,
             label = TRUE) +
             ggtitle( "Map of cells with clusters versus Outlier status")
  )
  
  outlier_vs_cluster_table = table( merge_seurat_object$outlier, merge_seurat_object$seurat_clusters)
  outlier_vs_cluster_table %>% kable( caption = "Cell Outlier status of Condition versus Clusters") %>% kable_styling()
  
  outlier_vs_cluster_chisq_test = chisq.test( outlier_vs_cluster_table)
  corrplot::corrplot( outlier_vs_cluster_chisq_test$residuals, is.cor = FALSE)
}