...
invisible( lapply( names( topMarkers_heatmap), function( clusterName)
{
  ...
  if( selected_term_number> 0){  
    ...
    positive_ego_df$cluster = clusterName
    write.table( positive_ego_df, col.names = TRUE, row.names = FALSE, sep=";",
                 file= file.path( PATH_ANALYSIS_EXTRA_OUTPUT, paste0( "GOBPEnrichment_Cluster", clusterName, ".csv"))
    )
    
    # Plot the result
    positive_dotplot = dotplot( positive_ego, showCategory=10) + ggtitle("Enrichment of marker genes in BP GO terms (positive)", paste( "Cluster", clusterName))
    ggsave( positive_dotplot, file = file.path( PATH_ANALYSIS_EXTRA_OUTPUT, paste0( "GOBPEnrichment_PositiveMarkers_Cluster", clusterName, ".svg" )))
  ...
  if( selected_term_number> 0){  
    ...
    positive_ego_df$cluster = clusterName
    write.table( positive_ego_df, col.names = TRUE, row.names = FALSE, sep=";",
                 file= file.path( PATH_ANALYSIS_EXTRA_OUTPUT, paste0( "GOMFEnrichment_Cluster", clusterName, ".csv"))
    )
    
    # Plot the result
    positive_dotplot = dotplot( positive_ego, showCategory=10) + ggtitle( "Enrichment of positive markers in MF GO terms (positive)", paste( "Cluster", clusterName))
    ggsave( positive_dotplot, file = file.path( PATH_ANALYSIS_EXTRA_OUTPUT, paste0( "GOMFEnrichment_PositiveMarkers_Cluster", clusterName, ".svg" )))
    print( positive_dotplot
           )
  ...
  if( !is.null( positive_kegg)){
    ...
    if( sum( positive_kegg_result_df$p.adjust <= ENRICHMENT_GO_PVALUECUTOFF) > 0){
      positive_kegg_result_df$cluster = clusterName
      write.table( positive_kegg_result_df, col.names = TRUE, row.names = FALSE, sep=";",
                   file= file.path( PATH_ANALYSIS_EXTRA_OUTPUT, paste0( "KEGGEnrichment_Cluster", clusterName, ".csv")))
      kegg_dotplot = dotplot( positive_kegg) + ggtitle( paste("Enrichment of positive markers in KEGG pathways- Cluster", clusterName))
      ggsave( kegg_dotplot, file = file.path( PATH_ANALYSIS_EXTRA_OUTPUT, paste0( "KEGGEnrichment_PositiveMarkers_Cluster", clusterName, ".svg" )))
      print( kegg_dotplot)
    ...
    # Plot the results for the chosen KEGG Pathways (recomputing the multitest adjustment with limited list)
    ...
    if( sum( selected_positive_kegg_result_df$p.adjust <= ENRICHMENT_GO_PVALUECUTOFF) > 0){
      write.table( selected_positive_kegg_result_df, col.names = TRUE, row.names = FALSE, sep=";",
                   file= file.path( PATH_ANALYSIS_EXTRA_OUTPUT, paste0( "ChosenKEGGEnrichment_Cluster", clusterName, ".csv")))
      positive_kegg@result = selected_positive_kegg_result_df
      kegg_dotplot = dotplot( positive_kegg) + ggtitle( paste( "Enrichment of positive markers in chosen KEGG pathways - Cluster", clusterName))
      ggsave( kegg_dotplot, file = file.path( PATH_ANALYSIS_EXTRA_OUTPUT, paste0( "ChosenKEGGEnrichment_PositiveMarkers_Cluster", clusterName, ".svg" )))
      print( kegg_dotplot)      
