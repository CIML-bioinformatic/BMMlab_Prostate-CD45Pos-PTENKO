...
for( CLUSTER_GROUP in names( paramsEnv$CLUSTER_GROUP_LIST)){
  ...
  assign( "PATH_ANALYSIS_OUTPUT" , file.path( paramsEnv$ORIGINAL_PATH_ANALYSIS_OUTPUT, CLUSTER_GROUP, paste0( "resolution_",paramsEnv$CLUSTER_RESOLUTION)) , env = paramsEnv )
  ...
  rmarkdown::render( input = file.path( WORKING_DIR, "Report.Rmd"),
                   ...
                   output_file  = paste0( SCIENTIFIC_PROJECT_NAME, "_", EXPERIMENT_NAME, "_", ANALYSIS_STEP_NAME, "_Resolution", CLUSTER_RESOLUTION, ".html"),
                   quiet = FALSE)  
