...
invisible( lapply( names( topMarkers_heatmap), function( clusterName)
{
  ...
  if( selected_term_number > 0){  
    ...
    goterm_dotplot = dotplot( positive_ego, showCategory=10) + 
                      ggtitle("Enrichment of marker genes in BP GO terms", paste( "Cluster", clusterName))
    print( goterm_dotplot)
    ggsave( filename = file.path( PATH_ANALYSIS_EXTRA_OUTPUT, paste0( "GOBP_Enrichment_Cluster", clusterName," .svg")),
            plot = goterm_dotplot)
  ...
  if( selected_term_number> 0){  
    ...
    goterm_dotplot = dotplot( positive_ego, showCategory=10) + 
                        ggtitle("Enrichment of marker genes in MF GO terms", paste( "Cluster", clusterName))
    print( goterm_dotplot)
    ggsave( filename = file.path( PATH_ANALYSIS_EXTRA_OUTPUT, paste0( "GOMF_Enrichment_Cluster", clusterName," .svg")),
            plot = goterm_dotplot)
  ...
  if( !is.null( positive_kegg)){
    ...
    if( sum( positive_kegg_result_df$p.adjust <= ENRICHMENT_GO_PVALUECUTOFF) > 0){  
      kegg_doplot = dotplot( positive_kegg) + 
        ggtitle("Enrichment of marker genes in KEGG pathways", paste( "Cluster", clusterName))
      print( kegg_doplot)
      ggsave( filename = file.path( PATH_ANALYSIS_EXTRA_OUTPUT, paste0( "KEGGPathways_Enrichment_Cluster", clusterName," .svg")),
              plot = kegg_doplot)
    ...
    if( sum( selected_positive_kegg_result_df$p.adjust <= ENRICHMENT_GO_PVALUECUTOFF) > 0){
      ...
      kegg_doplot = dotplot( positive_kegg) + 
        ggtitle("Enrichment of marker genes in chosen KEGG pathways", paste( "Cluster", clusterName))
      print( kegg_doplot)
      ggsave( filename = file.path( PATH_ANALYSIS_EXTRA_OUTPUT, paste0( "ChosenKEGGPathways_Enrichment_Cluster", clusterName," .svg")),
              plot = kegg_doplot)
    }else{
      cat("<BR>No enrichment found")
    }
