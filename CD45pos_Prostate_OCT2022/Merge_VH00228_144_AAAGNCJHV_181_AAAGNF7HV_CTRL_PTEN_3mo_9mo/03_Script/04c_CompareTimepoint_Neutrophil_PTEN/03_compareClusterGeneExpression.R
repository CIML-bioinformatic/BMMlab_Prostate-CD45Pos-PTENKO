  ...
  if( nrow( current_sample_cluster_markergenes_df) > 0){
    ...
    if( nrow( positive_current_sample_cluster_markergenes_df) > 30){
      ...
      if( selected_term_number> 0){  
        ...
        # Plot the results
        positive_dotplot = dotplot( positive_ego, showCategory=10) + ggtitle("Enrichment of marker genes in BP GO terms (positive)")
        ggsave( positive_dotplot, file = file.path( PATH_ANALYSIS_EXTRA_OUTPUT, paste0( "GOBPEnrichment_PositiveMarkers_Cluster", clusterid, ".svg" )))
    ...
    if( nrow( negative_current_sample_cluster_markergenes_df) > 30){
      ...
      if( selected_term_number> 0){  
        ...
        # Plot the results
        negative_dotplot = dotplot( negative_ego, showCategory=10) + ggtitle("Enrichment of marker genes in BP GO terms (negative)")
        ggsave( negative_dotplot, file = file.path( PATH_ANALYSIS_EXTRA_OUTPUT, paste0( "GOBPEnrichment_NegativeMarkers_Cluster", clusterid, ".svg" )))
        print( negative_dotplot)
