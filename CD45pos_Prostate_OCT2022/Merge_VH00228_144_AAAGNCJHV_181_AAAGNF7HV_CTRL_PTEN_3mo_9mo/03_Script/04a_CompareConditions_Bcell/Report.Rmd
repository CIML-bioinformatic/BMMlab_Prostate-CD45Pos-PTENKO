...
read_chunk( path=file.path( WORKING_DIR, "04_compareChosenClusterSetsGeneExpression.R"));
read_chunk( path=file.path( WORKING_DIR, "05_compareGeneModuleExpression.R"));
read_chunk( path=file.path( WORKING_DIR, "06_scigenexAnalysis.R"));
...
```{r rmd_sample_vs_cluster_distribution}
<<sample_vs_cluster_distribution>>
```

### Condition versus cluster
```{r rmd_condition_vs_cluster_distribution}
<<condition_vs_cluster_distribution>>
```

### Cluster versus condition
```{r rmd_cluster_vs_condition_distribution}
<<cluster_vs_condition_distribution>>
```

### Condition versus cell type
```{r rmd_condition_vs_celltypeL1_distribution}
<<condition_vs_celltypeL1_distribution>>
```

### Cluster versus cell type
```{r rmd_clusters_vs_celltypeL1_distribution}
<<clusters_vs_celltypeL1_distribution>>
```



## Analysis of clusters marker genes between conditions

```{r rmd_compare_cluster_expression}
<<compare_cluster_expression>>
```

## Analysis of clusters marker genes between chosen sets of clusters {.tabset .tabset-fade}

```{r rmd_compare_chosen_clusters_sets_expression}
if( exists( "CHOSEN_CLUSTERS_COMPARISONS_LIST")){
  <<compare_chosen_clusters_sets_expression>>
}else{
  cat("<BR>No comparison defined.")
}
```
