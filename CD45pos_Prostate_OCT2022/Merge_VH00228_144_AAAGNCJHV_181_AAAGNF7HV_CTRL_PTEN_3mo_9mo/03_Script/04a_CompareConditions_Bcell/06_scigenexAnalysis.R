...
scigenex_analysis <- filter_cluster_size(scigenex_analysis, min_cluster_size = 10)
...
scigenex_analysis <- top_genes( scigenex_analysis, top = 50)

Idents( merge_seurat_object) = "seurat_clusters"
plot_ggheatmap( scigenex_analysis, ident = Idents( merge_seurat_object)) + ggtitle( "Gene modules against Seurat Clusters")

Idents( merge_seurat_object) = "orig.condition"
plot_ggheatmap( scigenex_analysis, ident = Idents( merge_seurat_object)) + ggtitle( "Gene modules against Condition")
...
plot_ggheatmap( scigenex_analysis, ident = Idents( merge_seurat_object))  + ggtitle( "Gene modules against Cluster x Condition")
...
for( current_module in names( scigenex_analysis@top_genes)){
  ...
  gene_list_name = paste( "GeneModule", current_module, sep="_")
  
  merge_seurat_object = AddModuleScore( merge_seurat_object, features = gene_list, name = gene_list_name)
  print(
    FeaturePlot( merge_seurat_object, features = paste0( gene_list_name, "1")) + ggtitle ( paste( "Score of", gene_list_name))  + 
