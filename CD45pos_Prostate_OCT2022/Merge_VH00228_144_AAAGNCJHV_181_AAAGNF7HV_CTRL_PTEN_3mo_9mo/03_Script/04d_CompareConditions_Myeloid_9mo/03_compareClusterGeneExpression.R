...
for( clusterid in levels( Idents( merge_seurat_object))){
  ...
  cat("<BR>Number of cells in clusters: CTRL=", nb_cell_CTRL_in_cluster, "cells; PTEN=", nb_cell_PTEN_in_cluster)
  
  if( nb_cell_CTRL_in_cluster < 20 || nb_cell_PTEN_in_cluster < 20){
    cat("<BR>Too few cells in clusters to perform analysis")
    next
  }
  ...
  if( ! is.null( current_sample_cluster_markergenes_set)){
    ...
    if( !is.null( positive_ego)){
      ...
      # Plot the results if any, reducing term by similarity of terms
  ...
  if( ! is.null( current_sample_cluster_markergenes_set)){
    ...
    if( !is.null( positive_kegg)){
      ...
      if( sum( positive_kegg_result_df$p.adjust <= ENRICHMENT_GO_PVALUECUTOFF) > 0){  
        kegg_dotplot = dotplot( positive_kegg) + ggtitle("Enrichment of positive markers in KEGG pathways")
        ggsave( kegg_dotplot, file = file.path( PATH_ANALYSIS_EXTRA_OUTPUT, paste0( "KEGGEnrichment_Cluster", clusterid, ".svg" )))
        print( kegg_dotplot)
      }else{
