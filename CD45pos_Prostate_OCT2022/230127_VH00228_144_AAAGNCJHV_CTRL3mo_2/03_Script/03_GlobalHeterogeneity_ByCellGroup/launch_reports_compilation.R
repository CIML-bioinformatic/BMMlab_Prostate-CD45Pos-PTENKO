...
for( CLUSTER_GROUP in names( paramsEnv$CLUSTER_GROUP_LIST)){
  ...
  for( FINDCLUSTERS_RESOLUTION in c( 0.8, 1.0, 1.2)){
  
    assign( "CLUSTER_GROUP" , CLUSTER_GROUP , env = paramsEnv )
    assign( "FINDCLUSTERS_RESOLUTION" , FINDCLUSTERS_RESOLUTION , env = paramsEnv )
    assign( "PATH_ANALYSIS_OUTPUT" , file.path( paramsEnv$ORIGINAL_PATH_ANALYSIS_OUTPUT, CLUSTER_GROUP, paste0("Resolution_", FINDCLUSTERS_RESOLUTION)) , env = paramsEnv )
    ...
    rmarkdown::render( input = file.path( WORKING_DIR, "Report.Rmd"),
                       ...
                       output_file  = paste0( SCIENTIFIC_PROJECT_NAME, "_", EXPERIMENT_NAME, "_", ANALYSIS_STEP_NAME, "_ClusterGroup", CLUSTER_GROUP, ".html"),
                       quiet = FALSE)
  }
}
