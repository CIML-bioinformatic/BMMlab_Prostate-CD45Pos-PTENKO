...
cellsData = cbind( "Cell" = colnames( sc10x), # cell names from rownames conflict with search field in datatable
                   ...
                   "percent.mito" = as.numeric(format(sc10x[["percent.mito", drop = TRUE]], digits = 5)),
                   "percent.ribo" = as.numeric(format(sc10x[["percent.ribo", drop = TRUE]], digits = 5)),
...
lypanel_mitos = plotViolinJitter( cellsData,
                                  xAxisFormula = ~as.numeric( Cluster),
...
lypanel_ribos = plotViolinJitter( cellsData,
                                  xAxisFormula = ~as.numeric( Cluster),
