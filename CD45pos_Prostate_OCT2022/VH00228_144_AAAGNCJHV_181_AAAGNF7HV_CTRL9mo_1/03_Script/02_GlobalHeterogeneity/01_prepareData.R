...
sc10x = CreateSeuratObject( counts = Read10X( PATH_10XFILES),
...
#matchMonitoredGenes = match( toupper( unlist( MONITORED_GENES)), toupper( rownames( LayerData( sc10x))));
matchMonitoredGenes = match( ( unlist( MONITORED_GENES)), ( rownames( LayerData( sc10x))));
...
MONITORED_GENES = relist( rownames( LayerData( sc10x))[ matchMonitoredGenes ], skeleton = MONITORED_GENES); # Does not work with NULL list elements (removed earlier)
...
# #matchModulesGenes = match( toupper( unlist( MODULES_GENES)), toupper( rownames( LayerData( sc10x))));
# matchModulesGenes = match( ( unlist( MODULES_GENES)), ( rownames( LayerData( sc10x))));
...
# # Replace names by names found in assay (correcting eventual case mismatch, NA for not found)
# MODULES_GENES = relist( rownames( LayerData( sc10x))[ matchModulesGenes ], skeleton = MODULES_GENES); # Does not work with NULL list elements (removed earlier)
# # Finally remove names that did not match
...
mito.genes = grep( pattern = "^mt-", x = rownames( LayerData(object = sc10x, layer = "counts")), value = TRUE, ignore.case = TRUE)
...
ribo.genes = grep(pattern = "^rps|^rpl",  x = rownames(LayerData(object = sc10x, layer = "counts")), value = TRUE, ignore.case=TRUE)
...
table( sc10x$outlier, sc10x$scDblFinder) %>% kable( caption = "Outliers versus doublets tables") %>% kable_styling( full_width = FALSE)
